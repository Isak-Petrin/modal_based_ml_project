...
def load_samples(data_path, label=None):
    ext = os.path.splitext(data_path)[1].lower()

    if ext == '.pkl':
        return parse_pickle_char(data_path, label=label, use_dims=(0, 1))
    else:
        return parse_uji_penchar(data_path, chars_to_keep=[label])
...
def parse_pickle_char(filepath, label=None, use_dims=(0, 1)):
    """
    Load handwriting samples from a pickle file.

    Expected format:
        pickle file contains a list of arrays
        each array has shape (T, 3) or (T, D)
        where rows are already point offsets

    Args:
        filepath: path to .pkl file
        label: optional character label
        use_dims: tuple/list of dimensions to keep, default (0, 1)

    Returns:
        samples: list of dicts with same structure as parse_uji_penchar()
    """
    ...
    for i, arr in enumerate(loaded):
        ...
        # convert object dtype if needed
        if arr.dtype == object:
            arr = arr.astype(np.float32)
        else:
            arr = arr.astype(np.float32)

        # keep only x/y offsets
        offsets = arr[:, list(use_dims)]

        # skip degenerate sequences
        if offsets.ndim != 2 or offsets.shape[0] < 2:
            continue

        # since data already stores offsets, reconstruct trajectory by accumulation
        trajectory = np.cumsum(offsets, axis=0)

        samples.append({
            'label': label,
            'writer': f'pkl_{i}',
            'strokes': [trajectory],   # placeholder for compatibility
            'trajectory': trajectory,
            'offsets': offsets,
        })
...
def main():
    # --- Configuration ---
    DATA_PATH = 'data/W.pkl'   # <-- adjust path
    DIGIT = 'W'                # character label
    K = 20                     # number of regimes
    NUM_STEPS = 500            # SVI training steps
    LR = 0.005                 # learning rate

    print(f'=== AR-HMM for character "{DIGIT}" with K={K} regimes ===\n')

    # --- Load pickle data ---
    print('Loading dataset...')
    samples = parse_pickle_char(DATA_PATH, label=DIGIT, use_dims=(0, 1))
    print(f'  Found {len(samples)} samples for character "{DIGIT}"')
    ...
    for i in range(min(6, len(sequences_train))):
        ...
        plot_segmented_trajectory(
            ...
            title=f'Character "{DIGIT}" — sample {i} ({samples[i]["writer"]})',
            ax=axes[i]
        )
    ...
    for k in range(K):
        ...
        print(f'  Regime {k}: mu = {mu[k].round(3)} '
              f'(denormalized: dx={denorm_mu[0]:.3f}, dy={denorm_mu[1]:.3f})')
